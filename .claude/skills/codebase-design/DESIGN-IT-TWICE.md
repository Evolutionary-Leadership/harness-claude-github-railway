...
deepening candidate, use this parallel sub-agent pattern. Based on "Design
It Twice" (Ousterhout): your first idea is unlikely to be the best.
...
Spawn three or more sub-agents in parallel. Each must produce a
...
independent of the user-facing problem-space explanation in step 1. Give
each agent a different design constraint:

- Agent 1: "Minimize the interface; aim for 1 to 3 entry points max.
...
Include both the [SKILL.md](./SKILL.md) vocabulary and the project's
domain vocabulary from `docs/GLOSSARY.md` in the brief, so each sub-agent
names things consistently with the architecture language and the domain
language.
...
propose a hybrid. Be opinionated; the user wants a strong read, not a
menu.
